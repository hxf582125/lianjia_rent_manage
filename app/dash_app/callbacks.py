...
def register_callbacks(dash_app):
    ...
    @dash_app.callback(
        ...
        [State('single-batch-selector', 'options')],
        prevent_initial_call=False
    )
    def update_single_batch_charts(selected_batch, dimension, batch_options):
        """
        更新单批次分析的所有图表
        ...
        参数:
            ...
            batch_options: 批次选项列表，用于获取格式化显示文本
        ...
        count_data = HouseDAO.get_distribution_by_dimension(
            selected_batch, dimension
        )
        
        # 2. 获取租金区间分布数据
        price_data = HouseDAO.get_price_distribution(selected_batch)
        
        # 3. 获取面积区间分布数据
        area_data = HouseDAO.get_area_distribution(selected_batch)
        ...
        count_fig = create_bar_chart(
            count_data,
            title=f'{dim_name}维度房源数量分布（批次：{formatted_batch}）',
            x_label=dim_name,
            y_label='房源数量',
            top_n=top_n
        )
        ...
        price_fig = create_bar_chart(
            price_data,
            title=f'租金区间分布（批次：{formatted_batch}）',
            x_label='租金区间（元/月）',
            y_label='房源数量'
        )
        ...
        area_fig = create_bar_chart(
            area_data,
            title=f'面积区间分布（批次：{formatted_batch}）',
            x_label='面积区间（㎡）',
            y_label='房源数量'
        )
    ...
    def update_multi_batch_charts(selected_batches):
        """
        更新多批次对比的所有趋势图
        ...
        # 获取各批次的统计数据
        batch_stats = HouseDAO.get_batch_stats(selected_batches)
